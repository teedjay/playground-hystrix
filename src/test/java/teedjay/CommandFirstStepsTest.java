...
public class CommandFirstStepsTest {
    ...
    public void testSynchronousAndExpectAllToFail() {
        assertEquals(new CommandFirstSteps("World", 1).execute(), "TimedOut World, waiting 1ms was probably too short");
        assertEquals(new CommandFirstSteps("Bob", 1).execute(), "TimedOut Bob, waiting 1ms was probably too short");
    }
    ...
    public void testAsynchronous() throws ExecutionException, InterruptedException, TimeoutException {
        ...
        assertEquals(f1.get(10, TimeUnit.SECONDS), "Hello World!");
        assertEquals(f2.get(10, TimeUnit.SECONDS), "Hello Bob!");
    }
    ...
    private static boolean timedOut(Future<String> future) {
        try {
            return future.get(10, TimeUnit.SECONDS).contains("TimedOut");
        } catch (Exception ex) {
            return false;
        }
    }
